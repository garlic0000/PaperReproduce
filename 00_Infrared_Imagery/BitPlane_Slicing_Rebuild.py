...
def getslicing_pos(img, pos):
    # 获取某一比特平面 pos为第一平面 至 第八平面
    img_pos = img.copy()
    height, width = img.shape
    for i in range(height):
        for j in range(width):
            temp = img[i, j] // int(pow(2, pos - 1))
...
def main_getslicing_8_all():
    ...
    getslicing_8_all(img2)
    getslicing_8_all(img3)
    getslicing_8_all(img4)


def check_rebuild_slicing_8(img):
    # 测试比特重建函数
    img_8 = getslicing_8_all(img)
    img_re = rebuild_slicing_8(img_8)
    imgs = [img, img_re]
...
def main_rebuild_slicing_8():
    ...
    img3 = cv.imread("../Picture/Data-04-logo1b.jpg", cv.IMREAD_GRAYSCALE)
    img4 = cv.imread("../Picture/INFRARED.png", cv.IMREAD_GRAYSCALE)
    check_rebuild_slicing_8(img2)
    check_rebuild_slicing_8(img3)
    check_rebuild_slicing_8(img4)
